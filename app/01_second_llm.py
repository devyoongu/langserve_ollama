...
def create_second_chain():
    ...
    prompt = load_prompt("prompts/sql-convert.yaml", encoding="utf-8")
    llm = ChatOllama(model="EEVE-Korean-10.8B:latest", temperature=0)
...
if user_input:
    ...
    if first_chain is not None:
        ...
        response = first_chain.stream(user_input)
        ...
        # 2차 체인 실행: 1차 체인 응답(ai_answer)을 question으로 전달
        st.chat_message("assistant").write("Processing 2nd chain...")
        second_response = second_chain.stream({"context": "", "question": ai_answer})
        with st.chat_message("assistant"):
